...
- `DiseaseMobEffect` — `MobEffect` subclass that stores ordered attribute modifiers and a `feverOffset` double (not an attribute modifier). Read by `ColdSweatCompat` and `EffectRendererMixin`.
...
| `event/SymptomEvents.java` | Symptom-driven interactions (sore throat blocks eating, stomach cramps block healing, sharp pain blocks sleep) |
...
- **New MobEffect tiers use `DiseaseEffects.registerVariants()`.** It creates one `DiseaseMobEffect` per `Severity` in the window and scales modifiers by `debuffMult`.
